...
SELECT *
FROM visit_cte
ORDER BY customer_id, reverse_visit_number;
...
SELECT * FROM ranked WHERE best_rank = 1
UNION
SELECT * FROM ranked WHERE worst_rank = 1;
...
SELECT
    v.vendor_name,
    p.product_name,
    SUM(vi.price * 5 * c.customer_count) AS total_revenue
...
VALUES (
    9999, 'Apple Pie', '1 pc', 'unit', CURRENT_TIMESTAMP
);
